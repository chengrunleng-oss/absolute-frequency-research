...
若相位时间差定义为 $\Delta x=x_A-x_B$，在该符号约定下

$$
y(A/B)=\frac{d\Delta x}{dt}.
$$

因此 SDR 或 PPP 连续记录的相位差，其斜率就是平均分数频率差。若实验软件把相位差定义成 $x_B-x_A$，公式会多一个负号；必须先固定符号再处理数据。
...
2. **计量溯源流**：本地 HM 参考 GPS 接收机，远端 USN7 接收机参考 UTC(USNO)；双方 RINEX 文件经 Bernese PPP 得到 HM-UTC(USNO) 相位；Circular T 再把 UTC(USNO) 接到 TAI 和 SI 秒。
...
| 式 (4) 卫星链路不确定度 | Eq. (4)，PDF 第 7 页 | 把相位不稳定度换成 5 天频率不确定度 | `【论文给出】` + `【由上式代入】` | 端点相位误差、量纲换算 |
...
- SDR 零死时间记录该 `20 MHz` 信号的连续相位，利用相位斜率得到 Lu/HM 分数频率差。
...
UTC 与 TAI 只相差整数闰秒，频率标度间隔相同；整数相位 offset 不影响相邻 5 天相位点的斜率。Circular T 给出 `[UTC-UTC(k)]` 等相位差，再由相邻点变化得到频率关系。

Circular T 的 $d$ 描述 TAI **标度间隔**相对 SI 秒的偏差，而频率与周期互为倒数，所以一阶近似下频率偏差使用 $y_{\mathrm{TAI/SI}}\approx-d$。这就是复算中负号的来源。

### 10.3 论文式 (4)：卫星链路相位误差换成频率误差

**来源标注**：`【论文给出】` Eq. (4)，PDF 第 7 页。

$$
u=\frac{\sqrt{2}\,u_A}{\tau_0}
\left(\frac{\tau}{\tau_0}\right)^{-0.9}.
$$

- $u_A=0.2\ \mathrm{ns}$：Circular T 给出的 UTC(USNO) 卫星链路相位不稳定度。
- $\tau_0=5\ \mathrm{d}$：参考时间。
- 本测量 $\tau=5\ \mathrm{d}$。
- $\sqrt2$ 来自区间两个端点相位误差的独立合成。
- 相位时间误差除以区间长度后，单位由秒变成无量纲分数频率。
...
与表 2 的 `UTC(USNO)/TAI = 6.6e-16` 对应。指数 `-0.9` 是论文引用的推荐卫星链路模型，不是本文用本组数据拟合出的普适定律。
...
论文正文把蓝点写作 $-y(\mathrm{Lu/UTC(USNO)})$。负号来自原始 HM-relative-to-Lu 相位/频率符号与最终 Lu-relative-to-reference 定义相反：从 $y(\mathrm{HM/Lu})$ 切换到 $y(\mathrm{Lu/HM})$ 时，一阶近似要变号。实际复算应以原始相位定义为准，不能只凭图上“向上/向下”判断 correction 方向。
...
给出光钟运行时的统计稳定度
        |
Figure 2 + comb/phase equations
得到 Lu/HM 和 HM/UTC(USNO)
        |
Eq. (3)
...
| $f_o$ | 光频梳载波包络偏移频率 |
| $f_r$ | 光频梳重复频率 |
| $\Delta x$ | 两时标的相位时间差；斜率给频率差，正负依定义 |
