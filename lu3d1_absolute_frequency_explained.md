...
- SDR 零死时间记录该 `20 MHz` 信号的连续累积弧度相位。本文用 $\Phi_{20}(t)$ 表示完整累积相位，并把去掉标称线性相位后的剩余量记为 $\phi_{20}(t)$：

$$
\Phi_{20}(t)=2\pi f_{20,0}t+\phi_{20}(t),
$$

其中 $f_{20,0}$ 是约 `20 MHz` 的标称频率。因此两种导数必须区分：
...
所以完整相位 $\Phi_{20}$ 的导数给实际瞬时频率 $f_{20}$；只有剩余相位 $\phi_{20}$ 的导数才给相对标称值的频率偏差 $\delta f_{20}$。
...
以下混频关系写的是相对各自标称值的频率变化，因此使用剩余相位 $\phi_{20}$ 和 $\delta$ 记号：
...
**复现边界**：论文公开了链路结构，但没有公开 comb tooth index、DDS/beat 全部正负号、SDR 保存的是完整相位还是已去除标称斜率的剩余相位，以及完整 transfer equation。因此可以说明 $\Phi_{20}$、$\phi_{20}$、$f_{20}$ 和 $\delta f_{20}$ 的一般关系；不能仅凭论文文字唯一恢复从每个 SDR 样本到最终 Lu/HM 的完整比例系数，更不能简单“除以 20 MHz”。
...
| --- | --- | --- | --- | --- |
| SDR | 完整相位 $\Phi_{20}(t)$，或去除标称斜率后的剩余相位 $\phi_{20}(t)$ | rad | $f_{20}$ 或 $\delta f_{20}$，再到 $y(\mathrm{Lu/HM})$ | 先区分总相位/剩余相位，再处理 $2\pi$、混频符号、倍频与梳齿系数 |
...
Phi_20 -> f_20；phi_20 -> delta f_20
delta f_20 -> delta f_r -> delta nu_L
...
| $\Phi_{20}(t)$ | `20 MHz` 信号的完整累积相位；$(2\pi)^{-1}d\Phi_{20}/dt=f_{20}$ |
| $\phi_{20}(t)$ | 去掉 $2\pi f_{20,0}t$ 后的剩余相位；$(2\pi)^{-1}d\phi_{20}/dt=\delta f_{20}$ |
